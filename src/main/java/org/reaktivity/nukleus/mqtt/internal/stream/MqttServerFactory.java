    ...
    private final class MqttServer
    {
            ...
            MqttPublishFW publish)
        {
            ...
            if (route != null)
            {
                ...
                System.out.println("DECODE PUBLISH");
                MqttServerStream stream = streams.computeIfAbsent(topicKey, s ->
                {
                    System.out.println("NEW PUBLISH");
                    final MqttServerStream newStream = new MqttServerStream(newTarget, newRouteId, newInitialId, newReplyId,
                                                        0, topicName, null, MqttRole.SENDER);
            ...
            MqttSubscribeFW subscribe)
        {
            ...
            for (int progress = offset; progress < limit; progress = mqttSubscription.limit(), subscription.ackCount++)
            {
                ...
                if (route != null)
                {
                    ...
                    MqttServerStream stream = streams.computeIfAbsent(topicKey, s ->
                    {
                        final MqttServerStream newStream = new MqttServerStream(newTarget,
                            newRouteId, newInitialId, newReplyId, packetId, topicFilter, subscription,
                            MqttRole.RECEIVER);
        ...
        private class MqttServerStream
        {
                ...
                int subscriptionId)
            {
                ...
                if (this.roles.contains(MqttRole.SENDER))
                {
                    ...
                    role = MqttRole.SENDER;
                }
                else
                {
                    role = MqttRole.RECEIVER;
                }
                ...
                WindowFW window)
            {
                ...
                if (roles.contains(MqttRole.RECEIVER) && !MqttState.initialOpened(state))
                {
                ...
                ResetFW reset)
            {
                ...
                final long traceId = reset.traceId();
                final long authorization = reset.authorization();
                if (roles.contains(MqttRole.RECEIVER))
                {
